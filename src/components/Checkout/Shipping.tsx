...
const Shipping = ({
  defaults = {},
  onShippingTownChange,
  onShipToDifferentChange,
}: {
  defaults?: ShippingDefaults;
  onShippingTownChange?: (town: string) => void;
  onShipToDifferentChange?: (enabled: boolean) => void;
}) => {
  const [dropdown, setDropdown] = useState(false);
  const toggleDropdown = () => {
    const nextValue = !dropdown;

    setDropdown(nextValue);
    onShipToDifferentChange?.(nextValue);
    if (!nextValue) {
      onShippingTownChange?.("");
    }
  };
  ...
  return (
    <div className="bg-white shadow-1 rounded-[10px] mt-7.5">
      ...
      <div className={`p-4 sm:p-8.5 ${dropdown ? "block" : "hidden"}`}>
        {/* <div className="mb-5">
          ...
          <div className="relative">
            ...
            >
              ...
              {defaults.country && (
                <option value={defaults.country}>{defaults.country}</option>
              )}
            </select>
        ...
        <div className="mb-5">
          <label htmlFor="shippingTown" className="block mb-2.5">
          ...
          <input
            ...
            onChange={(event) => onShippingTownChange?.(event.target.value)}
            required={dropdown}
            className="rounded-md border border-gray-3 bg-gray-1 placeholder:text-dark-5 w-full py-2.5 px-5 outline-none duration-200 focus:border-transparent focus:shadow-input focus:ring-2 focus:ring-blue/20"
          />
        </div>
