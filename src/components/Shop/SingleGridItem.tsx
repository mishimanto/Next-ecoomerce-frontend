...
const SingleGridItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 min-h-[165px] sm:min-h-[205px] lg:min-h-[240px] xl:min-h-[270px] mb-3 sm:mb-4">
        <Image
          ...
          height={250}
          unoptimized
          className="max-h-[130px] w-auto sm:max-h-[170px] lg:max-h-[210px] xl:max-h-[250px]"
          style={{ height: "auto" }}
        />

        <div className="absolute left-0 bottom-0 flex w-full translate-y-full items-center justify-center gap-2 pb-3 ease-linear duration-200 group-hover:translate-y-0 group-focus-within:translate-y-0 group-active:translate-y-0 sm:gap-2.5 lg:pb-5">
          <button
            onClick={() => {
              openModal();
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full border border-gray-3 bg-white/95 text-dark shadow-2 backdrop-blur-sm ease-out duration-200 hover:border-blue hover:bg-blue hover:text-white sm:h-9 sm:w-9"
          >
            <svg
          ...
          <button
            ...
            className="inline-flex h-8 w-8 items-center justify-center gap-2 rounded-full bg-blue text-white shadow-2 ease-out duration-200 hover:bg-blue-dark sm:h-9 sm:w-9 lg:w-auto lg:px-4"
          >
            <FiShoppingCart size={17} className="lg:hidden" />
            <span className="hidden whitespace-nowrap text-custom-sm font-medium lg:inline">
              Add to Cart
            </span>
          </button>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2.5 mb-1.5 sm:mb-2">
        ...
        <p className="text-xs sm:text-custom-sm">({item.reviews})</p>
      </div>

      <h3 className="line-clamp-2 min-h-[38px] text-sm font-medium leading-snug text-dark ease-out duration-200 hover:text-blue mb-1 sm:min-h-0 sm:text-base sm:mb-1.5">
        <Link href={getProductUrl(item)}> {item.title} </Link>
      </h3>

      <span className="flex flex-wrap items-center gap-x-2 gap-y-0.5 font-medium text-sm sm:text-base lg:text-lg">
