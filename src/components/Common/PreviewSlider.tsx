...
const PreviewSliderModal = () => {
  const { closePreviewModal, images, initialSlide, isModalPreviewOpen } =
    usePreviewSlider();
  ...
  useEffect(() => {
    if (!isModalPreviewOpen || !sliderRef.current) {
      return;
    }

    sliderRef.current.swiper.slideTo(initialSlide, 0);
  }, [initialSlide, isModalPreviewOpen, images]);

  const previewImages = images.filter(Boolean);
