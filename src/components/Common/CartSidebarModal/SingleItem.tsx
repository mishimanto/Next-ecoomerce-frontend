...
const SingleItem = ({ item, removeItemFromCart }) => {
  ...
  const handleRemoveFromCart = () => {
    dispatch(removeItemFromCart(itemIdentity));
  };

  const updateQuantity = (quantity: number) => {
    dispatch(updateCartItemQuantity({ ...itemIdentity, quantity }));
  };
    ...
    <div className="flex items-start justify-between gap-4">
      <div className="flex w-full min-w-0 gap-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-custom-sm">Price: {formatCurrency(item.discountedPrice)}</p>
          <div className="mt-3 flex w-max items-center rounded-full bg-gray-2 p-1">
            <button
              type="button"
              onClick={handleDecreaseQuantity}
              disabled={item.quantity <= 1}
              aria-label="Decrease quantity"
              className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-dark ease-out duration-200 hover:text-[#ff7a1a] disabled:cursor-not-allowed disabled:text-dark-5"
            >
              <svg
            ...
            <span className="flex h-8 w-11 items-center justify-center text-custom-sm font-medium text-dark">
              {item.quantity}
            </span>
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-dark ease-out duration-200 hover:text-[#ff7a1a]"
            >
              <svg
                className="fill-current"
                width="18"
