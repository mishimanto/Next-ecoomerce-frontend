...
const SingleItem = ({ item }) => {
  ...
  const selectedOptions = Object.entries(item.selectedAttributes || {});
  const itemIdentity = {
    id: item.id,
  ...
  return (
    <div className="flex items-center py-5 px-7.5">
      ...
      <div className="min-w-[275px]">
        <div className="flex w-max items-center rounded-full bg-gray-2 p-1">
          <button
            type="button"
            onClick={handleDecreaseQuantity}
            disabled={item.quantity <= 1}
            aria-label="button for remove product"
            className="flex h-9 w-9 items-center justify-center rounded-full bg-white ease-out duration-200 hover:text-[#ff7a1a] disabled:cursor-not-allowed disabled:text-dark-5"
          >
            <svg
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-full bg-white ease-out duration-200 hover:text-[#ff7a1a]"
          >
            <svg
              className="fill-current"
              width="20"
      ...
      <div className="min-w-[200px]">
        <p className="text-dark">{formatCurrency(item.discountedPrice * item.quantity)}</p>
      </div>
